...
ratings100 = './recallData/ratings.csv'
ratings = pd.read_csv(ratings100).values
...
score = []
for i in range(len(recall_user)):   # len(recall_user)
    train_set = []
    if (i % 10000) == 0:
        print(i)
    ...
    score.append([ratings[i][0], ratings[i][1], train_res[0][0]])

output = pd.DataFrame(score)
output.to_csv('ratings_result.csv', header=False, index=False)